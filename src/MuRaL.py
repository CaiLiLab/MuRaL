...
class Network(nn.Module):
    ...
    def layer1out(self,x):
        ...
        if conv:
          ...
          out=x.clamp(min=0)
          print(out.shape, file=sys.stdout, flush=True)
          temp = out.data.cpu().numpy()
        else:
          print('you need to have CNN to visualize motifs', file=sys.stdout, flush=True)
        return temp
...
def Load_Data(train_file,test_file):
	...
	if train:
		...
		if embedding and word2vect_train:
			print('training word2vec model', file=sys.stdout, flush=True)
...
def Train_model():
	...
	best_hidden=True # hidden_list=[True,False]
	best_sigmaRNN=0.0029 #sigmaRNN=logsampler(10**-4,10**-1)
	best_xavier=True #xavier_List=[True,True,False]
	best_optim='Adagrad' #optim_list=['SGD','Adagrad','Adagrad']
	best_nummotif=16 #nummotif_list=[16]
	best_hidden_size=64 #hidden_size_list=[32,64]
	best_AUC=0
...
def run_deepRAM(parser):
	...
	print('embeddding: ' + str(embedding), file=sys.stdout, flush=True)
	...
	if train:
		  ...
		  #print('Automatic Calibration', file=sys.stdout, flush=True)
		  #best_hyperparameters=Calibration()
		  
		  print('Training 6 models using best hyper-parameters set', file=sys.stdout, flush=True)
		  model=Train_model()
		  print('Predicting sequence specificities', file=sys.stdout, flush=True)
		  model=test_predict()
		  
		  

	else:
		  print('Load Data', file=sys.stdout, flush=True)
		  Load_Data(train_data,test_data)
		  print('Predicting sequence specificities', file=sys.stdout, flush=True)
		  model=test_predict()
