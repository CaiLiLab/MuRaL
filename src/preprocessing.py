...
class HDF5Dataset1(Dataset):
    def __init__(self, data, seq_cols, cat_cols, output_col, h5f_path):
        ...
        self.cat_cols = cat_cols
        #self.cat_dims = [int(data[col].nunique()) for col in cat_cols]
...
def prepare_dataset1(bed_regions, ref_genome,  bw_files, bw_names, local_radius=5, local_order=1, distal_radius=50, distal_order=1, h5f_path='distal_data.h5', h5_chunk_size=1):
    ...
    if local_order > 1:
        ...
        #NOTE: maximum value (np.int32) !! 
        local_seq_cat2 = local_seq2.iseq4idx(list(range(local_seq2.shape[0]))).astype(np.int32)
    ...
    print('local_seq_cat2.shape:', local_seq_cat2.shape, local_seq_cat2.columns, categorical_features)
