...
distal_radius = int(sys.argv[3])
distal_order = int(sys.argv[4])
...
dataset, data_local, categorical_features = prepare_dataset(input_bed, ref_genome, bw_files,bw_names, radius, distal_radius, distal_order)
...
# CNN kernel size
if len(sys.argv)>6:
    cnn_kernel_size = int(sys.argv[6])
else:
    cnn_kernel_size = 12
...
# CNN output channels
if len(sys.argv)>7:
    cnn_out_channels = int(sys.argv[7])
else:
    cnn_out_channels = 50
...
# RNN hidden neurons
if len(sys.argv)>8:
    RNN_hidden_size = int(sys.argv[8])
else:
    RNN_hidden_size = 0
...
model1_path = sys.argv[10]

model2_path = sys.argv[11]

pred_outfile = sys.argv[12]
...
scaled_model.set_temperature(dataloader)
...
pred_df = pd.concat((input_bed.to_dataframe()[['chrom', 'start', 'end']], pred_df, pred_df2['prob']), axis=1)
pred_df.columns = ['chrom', 'start', 'end', 'mut_type','prob1', 'prob2']
#pred_df = pd.concat((input_bed.to_dataframe()[['chrom', 'start', 'end']], pred_df, pred_df2['prob'], pred_df3['prob']), axis=1)
