...
singularity exec --nv mural_v1.2.0.sif mural_indel train  --ref_genome data/seq.fa --train_data data/training.sorted.bed --experiment_name example1 > test1.out 2> test1.err

# After training, one can run the following command to get the best model per trial
singularity exec --nv mural_v1.2.0.sif mural_indel get_best_model ./results/example1/Train_*/progress.csv
...
singularity exec --nv mural_v1.2.0.sif mural_indel train --ref_genome data/seq.fa --train_data data/training.sorted.bed --validation_data data/validation.sorted.bed  --distal_radius 4000 --n_trials 3 --experiment_name example2 > test2.out 2> test2.err
...
singularity exec --nv mural_v1.2.0.sif mural_indel predict --ref_genome data/seq.fa --test_data data/testing.bed.gz    \
...
singularity exec --nv mural_v1.2.0.sif mural_indel transfer --ref_genome data/seq.fa --train_data data/training_TL.sorted.bed --validation_data data/validation.sorted.bed --model_path models/checkpoint_9/model --model_config_path models/checkpoint_9/model.config.pkl --train_all  --init_fc_with_pretrained --experiment_name example4 > test4.out 2> test4.err

# Example 5 (k-mer correlation evaluation):
singularity exec --nv mural_v1.2.0.sif mural_indel evaluate --pred_file testing.ckpt9.fdiri.tsv.gz --ref_genome data/seq.fa --kmer_length 2 --kmer_only --out_prefix test_kmer_corr > test5.out 2> test5.err

# # Example 6 (regional correlation evaluation):
singularity exec --nv mural_v1.2.0.sif mural_indel evaluate --pred_file testing.ckpt9.fdiri.tsv.gz --window_size 100000 --regional_only --out_prefix test_region_corr > test6.out 2> test6.err
