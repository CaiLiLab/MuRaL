...
from MuRaL.training import train_snv, train_indel
from MuRaL._version import __version__

import textwrap
#from torch.utils.tensorboard import SummaryWriter

def get_train_func(model_type):
    train_func_map = {
        'snv' : train_snv,
        'indel' : train_indel 
    }

    train_func =  train_func_map.get(model_type)
    if train_func is None:
        sys.exit("Error: model type should be 'snv' or 'indel', but got %s" % model_type)
    return train_func

def run_train(args, model_type):
    """
    ...
    train = get_train_func(model_type)
