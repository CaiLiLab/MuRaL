...
##############################
        ...
        #distal_out = torch.log((F.softmax(distal_out, dim=1)+ F.softmax(distal_out2, dim=1))/2)
        distal_out = (F.softmax(distal_out, dim=1) + F.softmax(distal_out2, dim=1))/2
        local_out = (F.softmax(local_out, dim=1) + F.softmax(local_out2, dim=1))/2
...
##############################
        ...
        out = torch.cat([local_out, local_out2, distal_out, distal_out2], dim=1)
...
##############################
        ...
        out = torch.log(torch.clamp(w[0]*local_out + w[1]*local_out2 + w[2]*distal_out + w[3]*distal_out2, min=1e-9))  
