...
class Network1_old(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, distal_radius, distal_order, distal_fc_dropout, n_class):
        ...
        Args:
            ...
            n_class: number of classes (labels)
        """
        super(Network1_old, self).__init__()
        self.n_class = n_class
    ...
    def forward(self, local_input, distal_input):
        ...
        if self.training == False and np.random.uniform(0,1) < 0.00001*distal_out.shape[0]:
...
class Network1(nn.Module):
    ...
    def forward(self, local_input, distal_input):
        ...
        distal_out = self.conv1(distal_input[:,:,(distal_input.shape[2]//2-100):(distal_input.shape[2]//2+100+1)]) #output shape: batch_size, L_out; L_out = floor((L_in+2*padding-kernel_size)/stride + 1) 
        jump_input = distal_out = self.maxpool1(distal_out)
...
##############################
        ...
        jump_input = distal_out2 = self.maxpool1_2(distal_out2)
        
        distal_out2 = self.RBs1_2(distal_out2)    
        assert(jump_input.shape[2] >= distal_out2.shape[2])
        distal_out2 = distal_out2 + jump_input[:,:,0:distal_out2.shape[2]]    
        distal_out2 = self.maxpool2_2(distal_out2)
        
        jump_input = distal_out2 = self.conv2_2(distal_out2)    
...
class Network2_old(nn.Module):
    ...
    def __init__(self,  emb_dims, no_of_cont, lin_layer_sizes, emb_dropout, lin_layer_dropouts, in_channels, out_channels, kernel_size, distal_radius, distal_order, distal_fc_dropout, n_class, emb_padding_idx=None):
        ...
        super(Network2_old, self).__init__()
...
class Network3(nn.Module):
    ...
    def __init__(self,  emb_dims, no_of_cont, lin_layer_sizes, emb_dropout, lin_layer_dropouts, in_channels, out_channels, kernel_size, distal_radius, distal_order, distal_fc_dropout, n_class, emb_padding_idx=None):
        ...
        self.avgpool1 =nn.AvgPool1d(5,5,2)
        ...
        self.maxpool1 = nn.MaxPool1d(7, 7, 3)
        # 1st set of residual blocks
        self.RBs1 = nn.Sequential(*[ResBlock(out_channels, kernel_size=rb1_kernel_size, stride=1, padding=(rb1_kernel_size-1)//2, dilation=1) for x in range(4)])
            

        self.maxpool2 = nn.MaxPool1d(5, 5, 2)# kernel_size, stride  
        self.conv2 = nn.Sequential(    
            nn.BatchNorm1d(out_channels),
            nn.Conv1d(out_channels, out_channels, kernel_size, 1, (kernel_size-1)//2),
            #nn.ReLU(),
    ...
    def forward(self, local_input, distal_input):
        ...
        # CNN layers for distal_input
        # Input data shape: batch_size, in_channels, L_in (lenth of sequence)
        distal_out = self.conv1(self.avgpool1(distal_input)) #output shape: batch_size, L_out; L_out = floor((L_in+2*padding-kernel_size)/stride + 1) 
        jump_input = distal_out = self.maxpool1(distal_out)
...
############################################3
        ...
        self.maxpool1_2 = nn.MaxPool1d(3, 3, 1)
        # 1st set of residual blocks
        self.RBs1_2 = nn.Sequential(*[ResBlock(out_channels, kernel_size=rb1_kernel_size, stride=1, padding=(rb1_kernel_size-1)//2, dilation=1) for x in range(2)])
            

        self.maxpool2_2 = nn.MaxPool1d(3, 3, 1)# kernel_size, stride  
        self.conv2_2 = nn.Sequential(    
            nn.BatchNorm1d(out_channels),
            nn.Conv1d(out_channels, out_channels, kernel_size, 1, (kernel_size-1)//2),
            #nn.ReLU(),
        ...
        # Separate FC layers for distal and local data
        self.distal_fc3_2 = nn.Sequential(
            nn.BatchNorm1d(cnn_fc_in_size),
            nn.Dropout(distal_fc_dropout), 
            nn.Linear(cnn_fc_in_size, n_class), 
            #nn.ReLU(),
...
##############################
        # Input data shape: batch_size, in_channels, L_in (lenth of sequence)
        distal_out2 = self.conv1_2(distal_input[:,:,(distal_input.shape[2]//2-100):(distal_input.shape[2]//2+100)]) #output shape: batch_size, L_out; L_out = floor((L_in+2*padding-kernel_size)/stride + 1) 
        jump_input = distal_out2 = self.maxpool1_2(distal_out2)
        
        distal_out2 = self.RBs1_2(distal_out2)    
        assert(jump_input.shape[2] >= distal_out2.shape[2])
        distal_out2 = distal_out2 + jump_input[:,:,0:distal_out2.shape[2]]    
        mid_out1 = distal_out2 = self.maxpool2_2(distal_out2)
        
        jump_input = distal_out2 = self.conv2_2(distal_out2)    
        distal_out2 = self.RBs2_2(distal_out2)
        assert(jump_input.shape[2] >= distal_out2.shape[2])
        distal_out2 = distal_out2 + jump_input[:,:,0:distal_out2.shape[2]]
        mid_out2 = distal_out2 = self.maxpool3_2(distal_out2)
        
        distal_out2 = self.conv3_2(distal_out2)
        distal_out2, _ = torch.max(distal_out2, dim=2)
        

        distal_out2 = self.distal_fc3_2(distal_out2)
...
############################################
    ...
    def forward(self, local_input, distal_input):
        ...
        # CNN layers for distal_input
        # Input data shape: batch_size, in_channels, L_in (lenth of sequence)
        distal_out = self.conv1(distal_input[:,:,(distal_input.shape[2]//2-100):(distal_input.shape[2]//2+100)]) #output shape: batch_size, L_out; L_out = floor((L_in+2*padding-kernel_size)/stride + 1) 
        jump_input = distal_out = self.maxpool1(distal_out)
...
class Network13(nn.Module):
    ...
    def __init__(self,  emb_dims, no_of_cont, lin_layer_sizes, emb_dropout, lin_layer_dropouts, in_channels, out_channels, kernel_size, distal_radius, distal_order, distal_fc_dropout, n_class, emb_padding_idx=None):
        ...
        self.conv1 = nn.Sequential(
            nn.BatchNorm1d(in_channels), # This is important!
            nn.Conv1d(in_channels, out_channels, kernel_size, stride=1), # in_channels, out_channels, kernel_size
            nn.MaxPool1d(maxpool_kernel_size, maxpool_stride),
            #nn.ReLU(),
        )
    ...
    def forward(self, local_input, distal_input):
        ...
        assert(jump_input.shape[2] >= distal_out.shape[2])
        distal_out = distal_out + jump_input[:,:,0:distal_out.shape[2]]    
        mid_out1 = distal_out = self.maxpool1(distal_out)
        
        jump_input = distal_out = self.conv2(distal_out)    
        distal_out = self.RBs2(distal_out)
        assert(jump_input.shape[2] >= distal_out.shape[2])
        distal_out = distal_out + jump_input[:,:,0:distal_out.shape[2]]
        mid_out2 = distal_out = self.maxpool2(distal_out)
...
class Network14(nn.Module):
    ...
    def forward(self, local_input, distal_input):
        ...
        mid_out1 = torch.mean(mid_out1, dim=2)
        mid_out2 = torch.mean(mid_out2, dim=2)
        distal_out = torch.cat((mid_out1, mid_out2, distal_out), dim=1)
