...
class Network4(nn.Module):
    ...
    def __init__(self,  emb_dims, no_of_cont, lin_layer_sizes, emb_dropout, lin_layer_dropouts, in_channels, out_channels, kernel_size, distal_radius, distal_order, distal_fc_dropout, n_class, emb_padding_idx=None):
        ...
        self.last_bn = nn.BatchNorm1d(n_class)
...
##############################
        ...
        out = self.last_bn(local_out + distal_out)*local_out2 
...
class Network8(nn.Module):
    ...
    def __init__(self,  emb_dims, no_of_cont, lin_layer_sizes, emb_dropout, lin_layer_dropouts, in_channels, out_channels, kernel_size, distal_radius, distal_order, distal_fc_dropout, n_class, emb_padding_idx=None):
        ...
        # 
        #self.models_w = nn.Parameter(torch.ones(4), requires_grad=True)
    ...
    def forward(self, local_input, distal_input):
        ...
        # Input data shape: batch_size, in_channels, L_in (lenth of sequence)
        distal_input0 = distal_input[:,:,(distal_input.shape[2]//2-100):(distal_input.shape[2]//2+100+1)].detach().clone()
        distal_out = self.conv1(distal_input0[:,0:self.in_channels,:]) #output shape: batch_size, L_out; L_out = floor((L_in+2*padding-kernel_size)/stride + 1) 
        jump_input = distal_out = self.maxpool1(distal_out)
...
##############################
        ...
        #distal_out = torch.log((F.softmax(mid_out1, dim=1) +F.softmax(mid_out2, dim=1) + F.softmax(distal_out, dim=1))/3)
        #distal_out = torch.log((F.softmax(distal_out, dim=1)+ F.softmax(distal_out2, dim=1))/2)
        distal_out = F.softmax(distal_out, dim=1) 
        distal_out2 = F.softmax(distal_out2, dim=1)
        local_out = F.softmax(local_out, dim=1)
        local_out2 = F.softmax(local_out2, dim=1)
        ...
        #w = F.softmax(self.models_w, dim=0)
        out = torch.stack((local_out, local_out2, distal_out, distal_out2), dim=1)
        out = torch.squeeze(self.conv1x1(out), dim=1)
...
class Network11(nn.Module):
    ...
    def __init__(self,  emb_dims, no_of_cont, lin_layer_sizes, emb_dropout, lin_layer_dropouts, in_channels, out_channels, kernel_size, distal_radius, distal_order, distal_fc_dropout, n_class, emb_padding_idx=None):
        ...
        self.conv1x1= nn.Sequential(
            #nn.ReLU(),
            nn.Dropout2d(p=0.01),
            nn.Conv1d(in_channels=conv1x1_in, out_channels=1, kernel_size=1),
        )
...
##############################
        ...
        if self.no_of_cont > 0:
            ...
            out = torch.stack((local_out, distal_out, distal_out2), dim=1)
        out = torch.squeeze(self.conv1x1(out), dim=1)
...
class Network13(nn.Module):
    ...
    def __init__(self,  emb_dims, no_of_cont, lin_layer_sizes, emb_dropout, lin_layer_dropouts, in_channels, out_channels, kernel_size, distal_radius, distal_order, distal_fc_dropout, n_class, emb_padding_idx=None):
        ...
        self.conv1x1= nn.Sequential(
            #nn.ReLU(),
            nn.Dropout2d(p=0.05),
            nn.Conv1d(in_channels=conv1x1_in, out_channels=1, kernel_size=3, padding=1),
        )
    ...
    def forward(self, local_input, distal_input):
        ...
        for lin_layer, dropout_layer, bn_layer in zip(self.lin_layers, self.droput_layers, self.bn_layers):
            local_out = F.leaky_relu(lin_layer(local_out))
            local_out = bn_layer(local_out)
            local_out = dropout_layer(local_out)
...
class Network14(nn.Module):
    ...
    def __init__(self,  emb_dims, no_of_cont, lin_layer_sizes, emb_dropout, lin_layer_dropouts, in_channels, out_channels, kernel_size, distal_radius, distal_order, distal_fc_dropout, n_class, emb_padding_idx=None):
        ...
        self.conv1x1= nn.Sequential(
            ...
            nn.BatchNorm1d(conv1x1_in),
            nn.Dropout2d(p=0.2),
            nn.Conv1d(in_channels=conv1x1_in, out_channels=n_class, kernel_size=n_class),
        )
...
##############################
        ...
        if self.no_of_cont > 0:
            ...
            out = torch.stack((local_out, distal_out, distal_out2), dim=1)
        out = torch.squeeze(self.conv1x1(out), dim=2)
