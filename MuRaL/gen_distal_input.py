...
def parse_arguments(parser):
    ...
    optional.add_argument('--chunk_size', type=int, metavar='INT', default=50000, 
                          help=textwrap.dedent("""
                          Bioseq read chunk size. Default: 50000. """ ).strip())
    
    optional.add_argument('--out_format', type=str, metavar='STR', default='zarr',  
                          help=textwrap.dedent("""
...
def main():
    ...
    * Output data 
    ...
    if out_format == 'zarr':
        test_zarr_path = get_zarr_path(test_file, bw_names, distal_radius, distal_order)     
        generate_zarr(test_bed, test_zarr_path, ref_genome, distal_radius, distal_order, bw_files, 5000) 
    else:
        test_h5f_path = get_h5f_path(test_file, bw_names, distal_radius, distal_order)
        #generate_h5f(test_bed, test_h5f_path, ref_genome, distal_radius, distal_order, bw_files, 1, chunk_size)
        generate_h5f2(test_bed, test_h5f_path, ref_genome, distal_radius, distal_order, bw_files, 1, chunk_size)
